...
public class ToPickup_LineDrawOnMapDAsync extends AsyncTask<String, Integer, List<List<HashMap<String,String>>> > {


    public fragment_goto_pickup fragmentPickupHandel;

    public ToPickup_LineDrawOnMapDAsync(fragment_goto_pickup fragmentPickupHandel) {
...
//                     Traversing through all the routes
                    ...
                    for (int i = 0; i < result.size(); i++) {
                        ...
                        fragmentPickupHandel.mGoogleMap.clear();
                    ...
                    fragmentPickupHandel.getActivity().runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(fragmentPickupHandel.context, "No Path Found", Toast.LENGTH_SHORT).show();
                        }
                    });
                }
            } else {
                fragmentPickupHandel.getActivity().runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(fragmentPickupHandel.context, "Error on path request", Toast.LENGTH_SHORT).show();
                    }
                });
            }
        }
